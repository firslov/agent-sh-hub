#!/usr/bin/env node
/**
 * Build the per-target ashub-server tarballs a release ships. Targets default
 * to linux-x64 + linux-arm64 (override via ASHUB_SERVER_TARGETS or CLI args).
 *
 * A target whose @vscode/ripgrep-<platform>-<arch> package is absent is
 * skipped rather than failing the build, so hooking this into electron:dist
 * can't block a dmg over a missing optional dep.
 */
